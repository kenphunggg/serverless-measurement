...
def collect_state(host: str, image: str, target_pods: int, repetition: int, state: str):
    ...
    count = 0
    while jobs_status[state]:
        get_prometheus_values_and_update_job(
            host, image, target_pods, state, repetition)
        time.sleep(0.5)
        count = count + 0.5
...
def collect_null_to_cold_process(host: str, image: str, target_pods: int, repetition: int, state: str):
    print("Scenario: {} - Started".format(state))
    while jobs_status[state]:
        # send Query to Prometheus to get measurement results
        get_prometheus_values_and_update_job(
            host, image, target_pods, state, repetition)
        # set rate of collecting measurement results, here after 0.2s, a value from Prometheus will be taken
        time.sleep(0.2)
...
def collect_curl(URL: str, host: str, image: str, target_pods: int, repetition: int, state: str, quality: str):
    print("Scenario: Measure {} - started".format(state))
    logging.info(f"Scenario: Measure {state} - started")
    count = 0
    while jobs_status[state]:
        get_curl_values_and_update_job(
            URL, host, image, target_pods, state, quality, repetition)
    ...
    print("Scenario: Measure {} - Ended".format(state))
    logging.info(f"Scenario: Measure {state} - Ended")
