...
# Normal lifecycle
def collect_cold_warm_disk(host: str, image: str, target_pods: int, repetition: int, event):
    timestamps = {}
    # k8s_API.config_image(WRONG_IMAGE_NAME)
    # k8s_API.update_deployment(target_pods, "null", "mec", DEFAULT_DIRECTORY + "/template1.yaml")
    remote_worker_call(DELETE_IMAGE_CMD, JETSON_USERNAME,
                       JETSON_IP, JETSON_PASSWORD)
    sleep(10)
...
def collect_life_cycle(host: str, image: str, target_pods: int, repetition: int, event):  # Normal lifecycle
    ...
    # NOTE: Null process
    # store when state NULL starts
    timestamps["null_state_start"] = time.time()
    # collect measurement data of state
   ...
   # Continue other jobs as below
    ...
    # curl requests are sent from inside a simple "ubuntu" pod
    ...
    # send curl requests processing a video
...
def curl_latency(host: str, image: str, list_quality: list, target_pods: int, repetition: int, event):
    ...
    logging.info(f"Start collecting {WARM_CPU_TO_ACTIVE_PROCESS}!")
    collect_curl(CURL_TRIGGER_TIME, host, image, target_pods,
                 repetition, WARM_CPU_TO_ACTIVE_PROCESS, "NONE")
    print("Finish collecting {}!".format(WARM_CPU_TO_ACTIVE_PROCESS))
    for quality in list_quality:
        # Response time from Warm CPU
        print("Start collecting {} with image quality is {} !".format(
            RESPOND_TIME_WARM_CPU, quality))
        jobs_status[RESPOND_TIME_WARM_CPU] = True
        collect_curl(CURL_RESPONSE_TIME.format(quality, "{}"), host,
                     image, target_pods, repetition, RESPOND_TIME_WARM_CPU, quality)
        print("Finish collecting {} with image quality is {} !".format(
            RESPOND_TIME_WARM_CPU, quality))
        sleep(10)
