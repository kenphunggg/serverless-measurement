...
class LLMMeasuring:
    def __init__(self, config, cluster_info: ClusterInfo):
        logging.info("Loading config of 'LLMMeasuring'")
    ...
    def get_text2text_warm(self):
        logging.info(
            "Scenario: Get 'Responsetime' of 'LLM service on text2text model' when pod in warm status"
        )
        for replica in self.replicas:
            for rep in range(1, self.repetition + 1, 1):
                ...
                result_file = CreateResultFile.text2text_warm(
                ...
                for i in range(self.curl_time):
                    ...
                    response = query_url(
                        url=f"http://{self.ksvc_name}.{self.namespace}/text2text?prompt=Write+me+a+poem+about+winter"
                    )
                    ...
                    if response is None:
                        ...
                        if not response["reply"]:
                            logging.warning(
                                "Fail when anaylyzing streaming using LLM service"
                            )
                            continue

                        with open(result_file, mode="a", newline="") as f:
                            result_value = [
                                response["performance"]["input_tokens"],
                                response["performance"]["output_tokens"],
                                response["performance"]["total_tokens"],
                                response["performance"]["tokens_per_second"],
                                response["performance"]["processing_time_second"],
                                response_time_ms,
                            ]
                            writer = csv.writer(f)
                ...
                PlotResult.get_text2text_warm(
                    result_file=result_file,
                    output_file=f"result/4_1_text2text/{self.hostname}/{self.arch}_{var.generate_file_time}_rep{rep}.png",
                )
    ...
    def get_text2image_warm(self):
        logging.info(
            "Scenario: Get 'Responsetime' of 'LLM service on text2image model' when pod in warm status"
        )
        ...
        for replica in self.replicas:
            for rep in range(1, self.repetition + 1, 1):
                ...
                # 1. Create result file
                result_file = CreateResultFile.text2image_warm(
                    nodename=self.hostname,
                    filename=f"{self.arch}_{var.generate_file_time}_rep{rep}.csv",
                )
                ...
                for i in range(self.curl_time):
                    logging.info(
                        f"Start measure response time of llm service - text2image model when pod in warm status [{i}/{self.curl_time}]"
                    )

                    start_time = time.time()
                    response = query_url(
                        url=f"http://{self.ksvc_name}.{self.namespace}/text2image?prompt=A+fox"
                    )
                    ...
                    if response is None:
                        ...
                        with open(result_file, mode="a", newline="") as f:
                            result_value = [
                                response["image_size_bytes"],
                                response["processing_time_second"],
                                response_time_ms,
                            ]
                            writer = csv.writer(f)
                            writer.writerow(result_value)
                ...
                PlotResult.get_text2image_warm(
                    result_file=result_file,
                    output_file=f"result/4_2_text2image/{self.hostname}/{self.arch}_{var.generate_file_time}_rep{rep}.png",
                )
    ...
    def get_warm_hardware_usage(self):
        ...
        for replica in self.replicas:
            for rep in range(1, self.repetition + 1, 1):
                ...
                def yolo_detection():
                    logging.info(
                        f"Start detecting image using yolo in {self.detection_time}"
                    )
                    query_url_post_image(
...
class PlotResult:
    @staticmethod
    def get_text2text_warm(result_file, output_file):
